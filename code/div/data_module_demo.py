...
class PressureTxtDataset(Dataset):
    def __init__(self, data_dir, mode='train', crop_size=128, target_height=128):
    ...
    def __getitem__(self, idx):
        ...
        if H < self.target_height:
            ...
            low_res = torch.nn.functional.pad(low_res, (0, 0, 0, pad_bottom), value=0)
            high_res = torch.nn.functional.pad(high_res, (0, 0, 0, pad_bottom), value=0)
        ...
        if self.mode == 'train':
            ...
            if W > self.crop_size:
                ...
                low_res = low_res[:, :, start_x : start_x + self.crop_size]
                high_res = high_res[:, :, start_x : start_x + self.crop_size]
            else:
                pad_right = self.crop_size - W
                low_res = torch.nn.functional.pad(low_res, (0, pad_right, 0, 0), value=0)
                high_res = torch.nn.functional.pad(high_res, (0, pad_right, 0, 0), value=0)
        else:
            # 验证/测试模式：补零到 128 的整数倍 (例如 1000 -> 1024)
            # 这样做是为了保证 U-Net多次下采样不会出错
            alignment = 128
            remainder = W % alignment
            if remainder != 0:
                pad_right = alignment - remainder
                low_res = torch.nn.functional.pad(low_res, (0, pad_right, 0, 0), value=0)
                high_res = torch.nn.functional.pad(high_res, (0, pad_right, 0, 0), value=0)
...
class SpecsDataModule(pl.LightningDataModule):
    ...
    def __init__(self, train_data_dir, val_data_dir, batch_size=4, num_workers=4, crop_size=128, **kwargs):
        super().__init__()
