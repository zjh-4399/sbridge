...
@BackboneRegistry.register("ncspp_s_decouple")
class NCSNpp_Small_Decouple(nn.Module):
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        self.mag_output_layer = nn.Conv2d(num_channels, 1, 1)
        self.pha_output_layer = nn.Conv2d(num_channels, 2, 1)
        ...
        for i_level in reversed(range(num_resolutions)):
            ...
            if progressive != 'none':
                if i_level == num_resolutions - 1:
                    ...
                    if i_level == 0:  # last combiner for phase
                        pha_modules = []
                        pha_modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                                        num_channels=in_ch, eps=1e-6))
                        pha_modules.append(conv3x3(in_ch, channels, bias=True, init_scale=init_scale))
                        pyramid_ch = channels
        ...
        self.pha_modules = nn.ModuleList(pha_modules)
    ...
    def forward(self, x, cond, time_cond):
        ...
        log_cond = torch.log(torch.norm(cond_, dim=1) + 1e-6)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if self.progressive != 'none':
                if i_level == self.num_resolutions - 1:  # decoder第一层
                    ...
                    if i_level == 0:  # for phase output
                        pha_pyramid = self.pyramid_upsample(pyramid)  # Upsample
                        pha_m_idx = 0
                        pha_pyramid_h = self.act(self.pha_modules[pha_m_idx](h))  # GroupNorm
                        pha_m_idx += 1
                        pha_pyramid_h = self.pha_modules[pha_m_idx](pha_pyramid_h)
                        pha_pyramid = pha_pyramid + pha_pyramid_h
        ...
        log_mag_h = self.mag_output_layer(mag_h)
        mag_h = torch.exp(log_mag_h.squeeze(1) + log_cond)
        com_h = self.pha_output_layer(pha_h)
        real_h, imag_h = com_h[:, 0], com_h[:, -1]
        pha_h = torch.atan2(imag_h, real_h)
        h = torch.stack([mag_h * torch.cos(pha_h), mag_h * torch.sin(pha_h)], dim=1)
